...
def _find_lib():
    ...
    # 3. Build directory relative to package (hteb/python/pytrine -> hteb/build)
    hteb_build = os.path.normpath(os.path.join(pkg_dir, "..", "..", "build"))
    search_dirs.append(hteb_build)

    # 4. hteb/ directory itself (in case .so was built there)
    hteb_dir = os.path.normpath(os.path.join(pkg_dir, "..", ".."))
    search_dirs.append(hteb_dir)
...
def get_lib():
    ...
    if path is None:
        raise OSError(
            ...
            "Build it with: make lib  (from hteb/python/)\n"
    ...
    lib.trine_encode_shingle.restype = None
