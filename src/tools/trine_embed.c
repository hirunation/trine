...
static int compute_embedding(const trine_file_t *model,
    ...
    if (trine_encode_shingle(text, text_len, encoding) != 0) return -1;
...
static void print_usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --stage2                 Use Stage-2 encoding (no .trine model file needed)\n"
        ...
        "  --block-diagonal         Use block-diagonal projection (4x60x60, for testing)\n"
        "\n"
        "Output Formats:\n"
...
int main(int argc, char **argv) {
    ...
    /* Check for simple --help/-h/--version before requiring model */
     ...
     * If argv[1] starts with '-', assume no model path was given
     * (e.g. --stage2 mode which doesn't need a .trine file). */
    ...
    while (i < argc) {
        ...
        if (strcmp(arg, "--info") == 0) {
            ...
            i += 2;
        } else if (strcmp(arg, "--stage2") == 0) {
            do_stage2 = 1;
            i++;
        } else if (strcmp(arg, "--depth") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "trine_embed: --depth requires a number\n");
                return 1;
            }
            s2_depth = atoi(argv[i + 1]);
            i += 2;
        } else if (strcmp(arg, "--s2-random") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "trine_embed: --s2-random requires a seed\n");
                return 1;
            }
            s2_random = 1;
            s2_seed = (uint64_t)strtoull(argv[i + 1], NULL, 0);
            i += 2;
        } else if (strcmp(arg, "--model") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "trine_embed: --model requires a file path\n");
                return 1;
            }
    ...
    /* Validate --model flag constraints */
    if (s2_model_path && !do_stage2) {
        fprintf(stderr, "trine_embed: --model requires --stage2\n");
        return 1;
    }
    if (s2_model_path && s2_random) {
        fprintf(stderr, "trine_embed: --model and --s2-random are mutually exclusive\n");
        return 1;
    }
    if (s2_block_diag && !do_stage2) {
        fprintf(stderr, "trine_embed: --block-diagonal requires --stage2\n");
        return 1;
    }
    if (s2_block_diag && s2_random) {
        fprintf(stderr, "trine_embed: --block-diagonal and --s2-random are mutually exclusive\n");
        return 1;
    }

    /* --stage2: Stage-2 embedding (no .trine model file needed) */
    if (do_stage2) {
        const char *text = NULL;
        size_t tlen = 0;

        if (file_path) {
            text = (const char *)read_file(file_path, &tlen);
            if (!text) return 1;
        } else if (n_positional > 0) {
            text = positional[0];
            tlen = strlen(text);
        } else {
            fprintf(stderr, "trine_embed: --stage2 requires text (positional or -f)\n");
            return 1;
        }

        trine_s2_model_t *s2m;
        if (s2_model_path) {
            s2m = trine_s2_load(s2_model_path);
            if (!s2m) {
                fprintf(stderr, "trine_embed: failed to load Stage-2 model '%s'\n",
                        s2_model_path);
                if (file_path) free((void *)text);
                exit(1);
            }
        } else if (s2_block_diag) {
            int K = TRINE_PROJECT_K;
            uint32_t n_cells = 4;
            uint64_t topo_seed = 42;
            size_t block_bytes = (size_t)K * TRINE_S2_N_CHAINS
                                 * TRINE_S2_CHAIN_DIM * TRINE_S2_CHAIN_DIM;
            uint8_t *block_weights = (uint8_t *)malloc(block_bytes);
            if (!block_weights) {
                fprintf(stderr, "trine_embed: allocation failed for block-diagonal weights\n");
                if (file_path) free((void *)text);
                return 1;
            }
            trine_projection_block_random(block_weights, K, 12345);
            s2m = trine_s2_create_block_diagonal(block_weights, K, n_cells, topo_seed);
            free(block_weights);
        } else if (s2_random) {
            s2m = trine_s2_create_random(512, s2_seed);
        } else {
            s2m = trine_s2_create_identity();
        }
        if (!s2m) {
            fprintf(stderr, "trine_embed: Stage-2 model creation failed\n");
            if (file_path) free((void *)text);
            return 1;
        }

        uint8_t out[TRINE_S2_DIM];
        int rc = trine_s2_encode(s2m, text, tlen, (uint32_t)s2_depth, out);
        if (rc != 0) {
            fprintf(stderr, "trine_embed: Stage-2 encode failed\n");
            trine_s2_free(s2m);
            if (file_path) free((void *)text);
            return 1;
        }
        ...
        if (verbose) {
            ...
            trine_s2_info(s2m, &info);
            const char *mode_str = s2_model_path ? "loaded" :
                                   s2_block_diag ? "block-diagonal" :
                                   info.is_identity ? "identity" : "random";
            fprintf(stderr, "Stage-2: %s model, %u cascade cells, depth=%d\n",
                    mode_str, info.cascade_cells, s2_depth);
            if (s2_model_path) {
                fprintf(stderr, "  Model file: %s\n", s2_model_path);
            }
        }

        /* Format output using embed_result_t for compatibility */
        embed_result_t emb;
        memset(&emb, 0, sizeof(emb));
        emb.trits = (uint8_t *)malloc(TRINE_S2_DIM);
        if (!emb.trits) {
            trine_s2_free(s2m);
            if (file_path) free((void *)text);
            return 1;
        }
        memcpy(emb.trits, out, TRINE_S2_DIM);
        emb.dims = TRINE_S2_DIM;
        emb.resolution = TRINE_RES_SHINGLE; /* Same 240-dim format */
        emb.ticks = s2_depth;
        emb.consistency = 2;

        print_embedding(&emb, fmt, text, tlen);
    ...
    /* Non-stage2 commands require a model path */
    if (!model_path) {
        fprintf(stderr, "trine_embed: a .trine model path is required as the first argument\n");
        fprintf(stderr, "Try '%s --help' for usage.\n", argv[0]);
        return 1;
    }
