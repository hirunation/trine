...
/* ═══════════════════════════════════════════════════════════════════════
 * Benchmark: Stage-2 Pipeline Throughput
 * ═══════════════════════════════════════════════════════════════════════ */

static void bench_stage2(int scale)
{
    printf("\n--- Stage-2 Pipeline Throughput ---\n");

    const char *text = "The quick brown fox jumps over the lazy dog";
    int len = (int)strlen(text);

    /* Identity model: projection-only (no cascade mixing) */
    {
        trine_s2_model_t *m = trine_s2_create_identity();
        int iters = 100000 / scale;
        uint8_t out[240];

        /* Warmup */
        for (int i = 0; i < iters / 100; i++)
            trine_s2_encode(m, text, (size_t)len, 0, out);

        double t0 = now_sec();
        for (int i = 0; i < iters; i++)
            trine_s2_encode(m, text, (size_t)len, 0, out);
        double elapsed = now_sec() - t0;
        bench_sink_i = (int)out[0];

        printf("  Identity (depth=0)     %15s encodes/sec  (%6.2f us/encode)\n",
               fmt_count((long long)(iters / elapsed)),
               (elapsed / iters) * 1e6);
        trine_s2_free(m);
    }

    /* Random model, depth=1 */
    {
        trine_s2_model_t *m = trine_s2_create_random(512, 42);
        int iters = 50000 / scale;
        uint8_t out[240];

        for (int i = 0; i < iters / 100; i++)
            trine_s2_encode(m, text, (size_t)len, 1, out);
        ...
        printf("  Random 512-cell (d=1)  %15s encodes/sec  (%6.2f us/encode)\n",
               fmt_count((long long)(iters / elapsed)),
               (elapsed / iters) * 1e6);
        trine_s2_free(m);
    }

    /* Random model, depth=4 */
    {
        trine_s2_model_t *m = trine_s2_create_random(512, 42);
        int iters = 50000 / scale;
        uint8_t out[240];
        ...
        for (int i = 0; i < iters / 100; i++)
            trine_s2_encode(m, text, (size_t)len, 4, out);
        ...
        printf("  Random 512-cell (d=4)  %15s encodes/sec  (%6.2f us/encode)\n",
               fmt_count((long long)(iters / elapsed)),
               (elapsed / iters) * 1e6);
        trine_s2_free(m);
    }

    /* Random model, depth=16 (stress) */
    {
        trine_s2_model_t *m = trine_s2_create_random(512, 42);
        int iters = 10000 / scale;
        uint8_t out[240];

        for (int i = 0; i < iters / 100; i++)
            trine_s2_encode(m, text, (size_t)len, 16, out);
        ...
        printf("  Random 512-cell (d=16) %15s encodes/sec  (%6.2f us/encode)\n",
               fmt_count((long long)(iters / elapsed)),
               (elapsed / iters) * 1e6);
        trine_s2_free(m);
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--quick") == 0) {
            ...
            phase4_only = 1;
        } else if (strcmp(argv[i], "--stage2-only") == 0) {
            stage2_only = 1;
        } else if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            printf("Usage: %s [--quick] [--encode-only] [--route-only] [--phase4-only] [--stage2-only]\n", argv[0]);
            ...
            printf("  --stage2-only  Only run Stage-2 pipeline benchmarks\n");
            return 0;
        } else {
