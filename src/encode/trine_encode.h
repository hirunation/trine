 ...
 *
 * All 240 channels are written on success. Channels not targeted by
 * any n-gram are zero-valued. Texts shorter than the minimum n-gram
 * size for a chain use a whole-text fallback hash, ensuring all
 * non-empty inputs produce at least one feature per chain.
 *
 * On allocation failure, channels is zeroed and -1 is returned.
 */
int trine_encode_shingle(const char *text, size_t len, uint8_t channels[240]);

/* Batch encode: encodes n texts into pre-allocated output array.
 * texts[i] points to text of length lens[i].
 * out must point to n * 240 bytes.
 * Returns 0 on success, -1 on any OOM.
 *
 * Optimisation over calling trine_encode_shingle() in a loop:
 * a single casefold buffer is allocated once (sized to the longest
 * input text) and reused for every encoding, eliminating per-call
 * malloc/free overhead for texts longer than 512 bytes. */
