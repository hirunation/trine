...
typedef struct {
    uint8_t *embeddings;    /* count * DEDUP_DIMS flat array */
    int count;
    int capacity;
} dedup_index_t;
 ...
 */
static void encode_line(const char *text, size_t len, uint8_t out[240])
{
    if (!text || len == 0) {
                     ...
                     int json_output)
{
    ...
    encode_line(text1, strlen(text1), emb1);
    encode_line(text2, strlen(text2), emb2);

    double sim = use_idf ? idf_lens_cosine(emb1, emb2, lens)
    ...
    printf("%s (%.3f)  lens=%s  threshold=%.2f%s\n",
 ...
 * dispatch via the append-index path. Standard scan uses static IDF. */
{
    ...
    if (use_routed) {
        ...
        while (fgets(buf, sizeof(buf), stdin)) {
            ...
            encode_line(text_content, text_len, emb);
                     ...
                     const char *save_idx_path)
{
    ...
    for (int i = 0; i < line_count; i++) {
        encode_line(lines[i], strlen(lines[i]),
...
int main(int argc, char **argv)
{
    ...
    if (is_check) {
        ...
        return cmd_check(positional[0], positional[1],
    ...
    if (is_scan) {
        ...
        return cmd_scan(threshold, &lens, verbose, quiet, use_idf,
    ...
    if (is_batch) {
        ...
        if (use_csidf && use_routed && save_index_path) {
            ...
            if (!lines) { trine_route_free(rt); return 1; }

            /* Encode and add all entries */
            for (int ei = 0; ei < line_count; ei++) {
                uint8_t emb[DEDUP_DIMS];
                encode_line(lines[ei], strlen(lines[ei]), emb);
                trine_route_add(rt, emb, NULL);
            }
        ...
        return cmd_batch(file_path, threshold, &lens,
    ...
    if (is_stats) {
        ...
        return cmd_stats(file_path, threshold, &lens, verbose, quiet,
