...
void trine_canon_normalize_identifiers(char *buf, size_t *len) {
     ...
     * We scan forward. At each lowercase->uppercase boundary, insert a
     * space before the uppercase char. This can grow the buffer, but
     * since we're working in-place in a buffer that started as a copy
     * of the input, we shift right. The caller's buffer was sized for
     * input + 1, so we must be careful. In practice, identifier
     * splitting can at most double the length, but the caller provided
     * out_cap >= len + 1. We work within *len.
     *
     * Strategy: build result in-place with a two-pass approach.
     * Pass 1: count insertions needed.
     * Pass 2: shift and insert from right to left.
     */
    size_t n = *len;
     ...
     * New length = n + inserts. This is safe because the buffer
     * was allocated with out_cap >= original_len + 1, and since
     * transforms only shrink or at worst mildly expand for identifiers,
     * we accept this within reason. We cap inserts to avoid overflow. */
    if (inserts > 0) {
        ...
        for (size_t i = n; i > 0; i--) {
            w--;
            buf[w] = buf[i - 1];
            if (i > 1 && is_lower(buf[i - 2]) && is_upper(buf[i - 1])) {
                w--;
                buf[w] = ' ';
            }
        }
...
int trine_canon_apply(const char *text, size_t len, int preset,
     ...
     * 2 * len + 1 (every char could be a camelCase boundary). */
    size_t need = (preset == TRINE_CANON_CODE) ? (2 * len + 1) : (len + 1);
    if (out_cap < need) {
    ...
    case TRINE_CANON_CODE:
        trine_canon_normalize_whitespace(out, &n);
        trine_canon_normalize_identifiers(out, &n);
        break;
