...
typedef struct {
    ...
    /* Stage-2 config */
    char             s2_model_path[TRINEDB_FILEPATH_MAX];
    int              s2_enabled;
    int              s2_block_diag;
    int              s2_depth;
    int              s2_adaptive_alpha;

    /* Runtime state */
    trine_route_t   *index;
    trine_s1_config_t config;
    trine_s2_model_t *s2_model;
    time_t           start_time;
    int              listen_fd;
    char             lens_name[32];
} trinedb_state_t;
...
{
    if (!g_state.s2_model) return "none";
    int mode = trine_s2_get_projection_mode(g_state.s2_model);
    switch (mode) {
...
static void handle_embed(int fd, const http_request_t *req)
{
    ...
    /* Encode Stage-1 */
    uint8_t emb[240];
    if (trine_encode_shingle(canon, canon_len, emb) != 0) {
        free(canon);
        send_error(fd, 500, "Internal Server Error", "Encode OOM");
        return;
    }
    float fill = trine_s1_fill_ratio(emb);
    free(canon);

    /* Optional Stage-2 projection */
    uint8_t s2_emb[240];
    int has_s2 = 0;
    if (g_state.s2_model) {
        if (trine_s2_encode_from_trits(g_state.s2_model, emb,
                                        (uint32_t)g_state.s2_depth,
                                        s2_emb) == 0) {
            has_s2 = 1;
        }
    }
...
static void handle_compare(int fd, const http_request_t *req)
{
    ...
    int rc_a = trine_encode_shingle(ca, len_a, emb_a);
    int rc_b = trine_encode_shingle(cb, len_b, emb_b);
    free(ca); free(cb);
    if (rc_a != 0 || rc_b != 0) {
        send_error(fd, 500, "Internal Server Error", "Encode OOM");
        return;
    ...
    if (g_state.s2_model) {
        ...
        int s2_ok = 0;
        if (trine_s2_encode_from_trits(g_state.s2_model, emb_a,
                                        (uint32_t)g_state.s2_depth, s2_a) == 0 &&
            trine_s2_encode_from_trits(g_state.s2_model, emb_b,
                                        (uint32_t)g_state.s2_depth, s2_b) == 0) {
            s2_ok = 1;
        }

        if (s2_ok) {
            float s2_sim = trine_s2_compare(s2_a, s2_b, NULL);

            if (g_state.s2_adaptive_alpha) {
                float adapt_sim = trine_s2_compare_adaptive_blend(
                    g_state.s2_model, emb_a, emb_b, s2_a, s2_b);
                n = snprintf(resp, sizeof(resp),
                    "{\"similarity\":%.6f,\"s2_similarity\":%.6f,"
                    "\"adaptive_similarity\":%.6f,"
                    "\"fill_a\":%.4f,\"fill_b\":%.4f}",
                    sim, s2_sim, adapt_sim, fill_a, fill_b);
            } else {
                n = snprintf(resp, sizeof(resp),
                    "{\"similarity\":%.6f,\"s2_similarity\":%.6f,"
                    "\"fill_a\":%.4f,\"fill_b\":%.4f}",
                    sim, s2_sim, fill_a, fill_b);
            }
        } else {
            n = snprintf(resp, sizeof(resp),
...
static void batch_add_cb(const char *obj, size_t obj_len, void *ctx_ptr)
{
    ...
    uint8_t emb[240];
    if (trine_encode_shingle(canon, canon_len, emb) != 0) {
        free(canon); free(tag); ctx->errors++;
        return;
    }
    free(canon);
...
static void handle_health(int fd)
{
    ...
    if (g_state.s2_model) {
        n = snprintf(resp, sizeof(resp),
            ...
            "\"stage2\":true,\"s2_projection\":\"%s\","
            "\"s2_depth\":%d,\"s2_adaptive_alpha\":%s}",
            TRINEDB_VERSION, count, uptime,
            s2_proj_mode_name(), g_state.s2_depth,
            g_state.s2_adaptive_alpha ? "true" : "false");
    } else {
        n = snprintf(resp, sizeof(resp),
            "{\"status\":\"ok\",\"version\":\"%s\","
            "\"index_count\":%d,\"uptime_seconds\":%ld,"
...
static void print_banner(void)
{
    ...
    if (g_state.s2_model) {
        fprintf(stderr, "  Stage-2:       enabled\n");
        fprintf(stderr, "  S2 projection: %s\n", s2_proj_mode_name());
        fprintf(stderr, "  S2 depth:      %d\n", g_state.s2_depth);
        if (g_state.s2_model_path[0])
            fprintf(stderr, "  S2 model:      %s\n", g_state.s2_model_path);
        else
            fprintf(stderr, "  S2 model:      (inline random)\n");
        if (g_state.s2_adaptive_alpha)
            fprintf(stderr, "  S2 adaptive:   enabled\n");
    } else {
        fprintf(stderr, "  Stage-2:       disabled\n");
    }
...
static void print_usage(const char *prog)
{
    fprintf(stderr,
        ...
        "  --stage2 PATH  Load trained .trine2 model for Stage-2 projection\n"
        "  --block-diagonal  Create inline random block-diagonal model (for testing)\n"
        "  --s2-depth N   Stage-2 cascade depth (default: 0)\n"
        "  --adaptive-alpha  Enable adaptive per-bucket alpha blending\n"
        ...
        "  %s --stage2 model.trine2        # load Stage-2 model\n"
        "  %s --block-diagonal             # random block-diagonal for testing\n"
        "\n",
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(arg, "--stage2") == 0 && i + 1 < argc) {
            snprintf(g_state.s2_model_path, sizeof(g_state.s2_model_path),
                     "%s", argv[++i]);
            g_state.s2_enabled = 1;
        }
        else if (strcmp(arg, "--block-diagonal") == 0) {
            g_state.s2_block_diag = 1;
            g_state.s2_enabled = 1;
        }
        else if (strcmp(arg, "--s2-depth") == 0 && i + 1 < argc) {
            g_state.s2_depth = atoi(argv[++i]);
            if (g_state.s2_depth < 0 || g_state.s2_depth > 64) {
                fprintf(stderr, "Error: --s2-depth must be 0-64\n");
                return 1;
            }
        }
        else if (strcmp(arg, "--adaptive-alpha") == 0) {
            g_state.s2_adaptive_alpha = 1;
        }
    ...
    /* Mutual exclusivity: --block-diagonal vs --stage2 PATH */
    if (g_state.s2_block_diag && g_state.s2_model_path[0]) {
        fprintf(stderr, "Error: --block-diagonal and --stage2 are "
                "mutually exclusive\n");
        return 1;
    }

    /* Adaptive alpha requires Stage-2 */
    if (g_state.s2_adaptive_alpha && !g_state.s2_enabled) {
        fprintf(stderr, "Error: --adaptive-alpha requires --stage2 or "
                "--block-diagonal\n");
        return 1;
    }

    /* Initialize Stage-2 model */
    if (g_state.s2_block_diag) {
        uint8_t *bw = calloc(TRINE_PROJECT_K * 4 * 60 * 60, 1);
        if (!bw) {
            fprintf(stderr, "Error: allocation failed for "
                    "block-diagonal weights\n");
            return 1;
        }
        trine_projection_block_random(bw, TRINE_PROJECT_K, 42);
        g_state.s2_model = trine_s2_create_block_diagonal(
            bw, TRINE_PROJECT_K, 4, 42);
        free(bw);
        if (!g_state.s2_model) {
            fprintf(stderr, "Error: failed to create block-diagonal model\n");
            return 1;
        }
        fprintf(stderr, "Created random block-diagonal Stage-2 model\n");
    } else if (g_state.s2_model_path[0]) {
        g_state.s2_model = trine_s2_load(g_state.s2_model_path);
        if (!g_state.s2_model) {
            fprintf(stderr, "Error: failed to load Stage-2 model from '%s'\n",
                    g_state.s2_model_path);
            return 1;
        }
        fprintf(stderr, "Loaded Stage-2 model: %s (projection: %s)\n",
                g_state.s2_model_path, s2_proj_mode_name());
    }
    ...
    /* Cleanup */
    trine_route_free(g_state.index);
    if (g_state.s2_model) trine_s2_free(g_state.s2_model);
