...
Stage-2 Support
---------------
Both TrineEmbeddings and TrineRetriever accept an optional ``stage2_model``
...
Usage::
    ...
    # Stage-2 embeddings:
    embeddings_s2 = TrineEmbeddings(stage2_model="trained.trine2", depth=0)
    vectors_s2 = embeddings_s2.embed_documents(["text1", "text2"])

    # Block-diagonal Stage-2 embeddings with adaptive alpha:
    embeddings_bd = TrineEmbeddings(
        stage2_model="trained_bd.trine2",
        block_diagonal=True,
        adaptive_alpha=[0.5] * 10,
    )
    vectors_bd = embeddings_bd.embed_documents(["text1", "text2"])
...
class TrineEmbeddings(LCEmbeddings):
    ...
    stage2_model : str or Stage2Model, optional
        ...
        When provided, Stage-2 encoding is used instead of Stage-1.
        Default is None (Stage-1 only).
    depth : int
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        Otherwise, uses Stage-1 shingle encoding.
...
class TrineRetriever(BaseRetriever):
    ...
    block_diagonal : bool
        ...
        See ``TrineEmbeddings`` for details.  Default False.
    adaptive_alpha : list of float, optional
        10-element per-S1-bucket alpha list for adaptive blending.
        See ``TrineEmbeddings`` for details.  Default None.
    ...
    def similarity_search(
        ...
        query_s2_emb = None
        if self._s2_encoder is not None:
            query_s2_emb = self._s2_encoder.encode(query)
        ...
        for i, stored_emb in enumerate(self._embeddings):
            ...
            if self._s2_encoder is not None and query_s2_emb is not None:
                # Compute Stage-2 embeddings for the stored document and
                # blend with Stage-1
                stored_s2_emb = self._s2_encoder.encode(
                    self._documents[i].page_content
                )
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        stage2_model : str or Stage2Model, optional
            A path to a ``.trine2`` file or a ``Stage2Model`` instance.
            When provided, similarity_search uses blended S1+S2 scoring.
        blend_alpha : float
            Blending weight for Stage-2 scoring
            (``alpha * S1 + (1 - alpha) * S2``).  Default 0.65.
        block_diagonal : bool
            If True, use block-diagonal projection mode for Stage-2.
            Default False.
        adaptive_alpha : list of float, optional
